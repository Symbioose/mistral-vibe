...
"audit this repo", "find all the bugs", "understand this codebase", "check that
everything is consistent", "compare these approaches", "migrate every usage of X".
YOUR job is to recognize that the ask is broad, deep, or repetitive, and to design
...
- `args` — the value passed as the tool's `args` input, verbatim (`None` if not provided). Use it to parameterize workflow instead of hardcoding.
