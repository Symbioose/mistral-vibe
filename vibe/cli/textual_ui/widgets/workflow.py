...
from vibe.core.workflow.models import AgentRunStatus
...
class WorkflowPhaseGroup(Vertical):
    ...
    def compose(self) -> ComposeResult:
        if self._title is not None:
            self._header = NoMarkupStatic("", classes="workflow-phase-header")
            yield self._header
...
class WorkflowCallMessage(ToolCallMessage):
    ...
    def compose(self) -> ComposeResult:
        with Vertical(classes="tool-call-container"):
            ...
            self._cat_box = Vertical(classes="workflow-cat")
            self._cat_box.display = False
            with self._cat_box:
                with Horizontal(classes="workflow-cat-strip"):
                    shades = list(_CAT_COLOR_CLASSES) * 2
                    random.shuffle(shades)
                    self._cat_pool = [
                        NonSelectableStatic(
                            KITTEN_ART,
                            classes=f"cat-art kitten-art workflow-cat-item {shade}",
                        )
                ...
                self._cat_label = NoMarkupStatic(
                    "", classes="cat-label kitten-label workflow-cat-label"
                )
    ...
    async def _ensure_phase(
        ...
        if group is None:
            group = WorkflowPhaseGroup(title, detail)
