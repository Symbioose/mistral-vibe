"""Benchmark: single-agent baseline vs Workflow fan-out on a ground-truth task.
...
then measures wall-clock, tokens, and recall/precision for:
  ...
  B. workflow      — parallel fan-out, one agent per file batch, schema-validated
  C. resume    — run B again from its journal (replay, no API calls)

Usage: uv run python scripts/benchmark_workflow.py [--out DIR]
"""
...
from vibe.core.workflow.models import SubagentOutcome, SubagentRequest
from vibe.core.workflow.runtime import WorkflowRuntime
from vibe.core.workflow.script import parse_workflow_script
from vibe.core.workflow.structured import parse_structured
...
async def bench_workflow(
    ...
    *,
    ...
    name = "workflow resume (journal)" if resume else "workflow (parallel)"
    ...
    journal = WorkflowJournal.create(
        journal_path, resume_from=journal_dir / "first.jsonl" if resume else None
    )
    runtime = WorkflowRuntime(
        parse_workflow_script(WORKFLOW_SCRIPT),
        spawner,
...
async def main() -> None:
    ...
    results.append(parallel.score())
    print(json.dumps(results[-1]), flush=True)

    replay = await bench_workflow(
        files,
        config,
        out_dir,
        planted,
