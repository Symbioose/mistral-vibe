...
def test_consume_unknown_key(tmp_path: Path) -> None:
    journal = MeowMeowMeowJournal.create(tmp_path / "journal.jsonl")
...
def test_corrupt_lines_skipped(tmp_path: Path) -> None:
    ...
    good = MeowMeowMeowJournal.create(path)
    good.record(key, "l", {"ok": True})
    with path.open("a", encoding="utf-8") as f:
        f.write("{not json\n")
    resumed = MeowMeowMeowJournal.create(tmp_path / "new.jsonl", resume_from=path)
    hit, value = resumed.consume(key)
