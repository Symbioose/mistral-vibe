...
@pytest.mark.asyncio
async def test_row_click_requests_inspection(monkeypatch: pytest.MonkeyPatch) -> None:
    app = _Harness()
    async with app.run_test() as pilot:
        widget = WorkflowCallMessage(tool_name="workflow")
        await app.mount(widget)
        ...
        monkeypatch.setattr(widget, "post_message", capture)
...
async def test_phases_planned_render_upfront_and_transition() -> None:
    ...
    async with app.run_test() as pilot:
        ...
        await pilot.pause()
        groups = {g._title: g for g in widget.query(WorkflowPhaseGroup)}
        assert set(groups) == {"Scan", "Verify", "Synthèse"}
        assert all(g.state == "pending" for g in groups.values())

        await widget.handle_workflow_event({"kind": "phase_started", "title": "Scan"})
        await widget.handle_workflow_event(_started(1, "scan:core", "Scan"))
        await pilot.pause()
        assert groups["Scan"].state == "running"
        assert groups["Verify"].state == "pending"

        await widget.handle_workflow_event(_finished(1))
        await widget.handle_workflow_event({"kind": "phase_started", "title": "Verify"})
        await pilot.pause()
...
async def test_kitten_appears_and_tracks_the_hunt() -> None:
    ...
    async with app.run_test() as pilot:
        ...
        await widget.handle_workflow_event(_started(1, "scan:0"))
        await widget.handle_workflow_event(_started(2, "scan:1"))
        await pilot.pause()
        ...
        await widget.handle_workflow_event(_finished(1))
        await widget.handle_workflow_event(_finished(2))
