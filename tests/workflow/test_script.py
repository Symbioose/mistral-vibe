...
from vibe.core.workflow.script import build_script_globals, parse_workflow_script
...
def test_fast_model_is_reserved() -> None:
    script = 'meta = {"name": "x", "description": "d"}\nfast_model = "oops"\n'
    with pytest.raises(
        WorkflowScriptError, match="'fast_model' is a workflow primitive"
    ):
        parse_workflow_script(script)
...
def test_no_top_level_await_rejected() -> None:
    ...
    with pytest.raises(WorkflowScriptError, match="never awaits anything at top level"):
        parse_workflow_script(script)
...
def test_sequential_agent_await_in_for_loop_rejected() -> None:
    ...
    with pytest.raises(WorkflowScriptError, match="ONE BY ONE"):
        parse_workflow_script(script)
