# Workflow benchmark — measured, live API
...
- **workflow** — parallel fan-out, one agent per batch of files,
  schema-validated JSON findings, merged in script,
- **resume** — the same workflow run re-invoked with `resume_from_run_id`.
...
| workflow (6 agents) | 29.3 | 40 014 | 6/0 | 8/8 | 100% | 100% |
| workflow resume (journal) | 0.0 | **0** | 0/6 | 8/8 | 100% | 100% |
...
| workflow (12 agents) | **28.2** | **115 307** | 12/0 | 18/18 | 100% | 100% |
| workflow resume (journal) | **0.0** | **0** | 0/12 | 18/18 | 100% | 100% |
...
| workflow (32 agents) | **37.6** | **305 908** | 32/0 | 48/48 | 100% | 100% |
| workflow resume (journal) | **0.0** | **0** | 0/32 | 48/48 | 100% | 100% |
...
| corpus | baseline tokens | baseline wall | workflow tokens | workflow wall |
|---|---|---|---|---|
...
```
uv run python scripts/benchmark_workflow.py --out bench_small
uv run python scripts/benchmark_workflow.py --files 36 --bugs 18 --batch 3 --fillers 14 --out bench_large
uv run python scripts/benchmark_workflow.py --files 96 --bugs 48 --batch 3 --fillers 14 --concurrency 16 --out bench_max
```
...
The benchmark drives the real engine — real Mistral API calls, one real
`AgentLoop` (explore profile) per agent, the real Workflow runtime/journal —
headless, without the TUI wrapper. The audited codebase is a synthetic corpus
the script generates deterministically (`m00.py`…`mNN.py`, small utility
functions, planted logic bugs at known positions), which is what makes
accuracy verifiable instead of declared: results are scored against
`ground_truth.json` written alongside the corpus.

To reproduce without re-running the 8-minute baseline, pass
`--skip-baseline`; the resume condition then lands at 0 tokens / 0.0 s.
